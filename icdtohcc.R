...
icd9cc <- apply(data.frame(paste("data/icd9/",list.files("data/icd9/"),sep = "")), 1, FUN = read.fwf, width = c(7, 4), header = F, stringsAsFactors = F)
icd10cc <- apply(data.frame(paste("data/icd10/",list.files("data/icd10/"),sep = "")), 1, FUN = read.fwf, width = c(7, 4), header = F, stringsAsFactors = F)

# Create a vector of year names based on the file names in the icd folders
years <- list()
years$icd9 <- as.numeric(substr(list.files("data/icd9/"), 0, 4))
years$icd10 <- as.numeric(substr(list.files("data/icd10/"), 0, 4))
...
# Row bind icd9 and icd10 from different years into respective dataframes
...
labels <- apply(data.frame(paste("data/labels/",list.files("data/labels/"),sep = "")), 1, FUN = readLines)
...
hierarchy <- apply(data.frame(paste("data/hierarchy/",list.files("data/hierarchy/"),sep = "")), 1, FUN = readLines)

# Create a vector of year names based on the file names in the icd folders
years <- substr(list.files("data/hierarchy/"), 0,4)
...
todrop <- str_extract(hierarchy$condition, "(?<=i\\=)([:print:]*)(?=;hcc)|(?<=STR\\()([:print:]*)(?= \\)\\);)")
...
# pt transform long to wide
# pt$hcc <- pt$cc
# pt <- reshape(pt, timevar = "cc", idvar = c("id", "admtdate"), direction = "wide", sep = "")

# assign zeros to na
# pt[is.na(pt)] <- 0
# assign one to non zero values for hcc columns
# pt[, 3:length(pt)][pt[, 3:length(pt)] != 0] <- 1

#############################################
######### Import Coefficients
#############################################
cf <- read.csv(file = "data/C2110H2R.csv", header = F, stringsAsFactors = F)
# transpose the data.frame
cf <- data.frame(t(cf))
colnames(cf) <- c("cc", "coeff")

# keep only the required coefficients
cf <- cf[grepl("INS_HCC", cf$cc),]
cf$cc <- as.numeric(str_extract(cf$cc, "[0-9]+"))

#############################################
######### Apply Coefficients to pt
#############################################

pt <- merge(pt, cf, all.x = T)

# assign zeros to unmatched coeff
pt$coeff[is.na(pt$coeff)] <- 0
pt$coeff <- as.numeric(paste(pt$coeff))

# calculate the score for all patients
ptscore <- aggregate(pt$coeff, by = list(pt$id, pt$admtdate), FUN = "sum", simplify = F)
colnames(ptscore) <- c("id", "admtdate", "score")
